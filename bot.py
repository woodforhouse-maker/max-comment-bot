...
def send_main_menu(user_id):
    ...
    send_message(
        user_id=user_id,
        text="Выберите действие — просто нажмите кнопку:",
        keyboard=keyboard_buttons
    )
...
def webhook():
    ...
    # === Обработка обычных сообщений боту ===
        ...
        if text and text.strip() == "📞 Связаться с мастером":
            send_message(
                ...
                text=(
                    ...
                    "Также вы можете заказать изделие через каталог — "
                    "нажмите «📋 Каталог» и выберите понравившееся."
                )
            )
            return jsonify({"ok": True}), 200
        ...
        if sender_id in pending_replies and text and not text.startswith("/"):
            ...
            # Приветствие + сразу главное меню с кнопками
            send_message(
                user_id=sender_id,
                text=(
